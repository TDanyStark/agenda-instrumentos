        ...
        <div id="contenedor-popup" class="relative p-4 w-full max-w-2xl max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <section class="p-4 md:p-8">
                    ...
                    <div class="mb-5">
                        <label for="duracion" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Duracion curso (minutos)</label>
...
<script>
    ...
    modalAddCourse.addEventListener('click', function(e) {
        if (!contenedorPopup.contains(e.target)) {
            changeModalState();
        }
    });

    // cuando le de click en el boton de crear curso que haga un fetch a la ruta /add-course
    // y que envie los datos del curso

    const btnGuardar = document.getElementById('btn-guardar');

    btnGuardar.addEventListener('click', () => {
        const nombreCurso = document.getElementById('nombre').value;
        const duracionCurso = document.getElementById('duracion').value;

        if (!nombreCurso || !duracionCurso) {
