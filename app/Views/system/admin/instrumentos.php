		...
		<table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
			<thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
				<tr>
					...
					<th scope="col" class="px-6 py-3">Instrumento</th>
			...
			<tbody>
				<?php foreach ($instruments as $instrument) : ?>
					<tr class="bg-white dark:bg-gray-800">
						<td class="px-6 py-4"><?= $instrument->InstrumentID ?></td>
						<td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white"><?= $instrument->InstrumentName ?></td>
						<td class="px-6 py-4">
							<button data-instrumentid="<?= $instrument->InstrumentID ?>" class="btn-delete text-red-600 hover:text-red-900" type="button">Eliminar</button>
						</td>
					</tr>
				<?php endforeach; ?>
			</tbody>
		...
		<div id="contenedor-popup" class="relative p-4 w-full max-w-2xl max-h-full">
			...
			<div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
				...
				<div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
					<h3 class="text-xl font-semibold text-gray-900 dark:text-white">Agregar instrumento</h3>
...
<script>
	...
	$nombre.addEventListener('input', () => {
		formData.InstrumentName = $nombre.value;
	});

	const renderFields = () => {
		$nombre.value = formData.InstrumentName;
	};
	...
	btnGuardar.addEventListener('click', async () => {
		try {
			...
			const response = await fetch('<?= base_url('api/add-instrument') ?>', {
				method: 'POST',
				headers: {
			...
			if (!response.ok) {
				...
				throw new Error(errorData.message || 'Error al agregar el instrumento');
			}
	...
	btnsDelete.forEach(btn => {
		btn.addEventListener('click', async function() {
			const id = btn.dataset.instrumentid;
			...
			try {
				...
				const response = await fetch('<?= base_url('api/delete-instrument') ?>', {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json'
					},
